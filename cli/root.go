// Package cli assembles the mealdb command tree from the mealdb
...
// NewApp assembles the kit application from the mealdb domain. The
...
//
// To add a command, declare it in mealdb/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := mealdb.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(mealdb.Domain{}).Register(app)
