...
describes what users get. Pick a format with `-o`, or let mealdb choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mealdb <command> -o table   # aligned columns for reading
mealdb <command> -o jsonl   # one JSON object per line, for piping
mealdb <command> -o json    # a single JSON array
mealdb <command> -o csv     # spreadsheet friendly
mealdb <command> -o tsv     # tab-separated
mealdb <command> -o url     # just the URL column
mealdb <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mealdb <command> --fields id,title,url
```
...
```bash
mealdb <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mealdb <command>            # a table, because this is a terminal
mealdb <command> | wc -l    # JSONL, because this is a pipe
```
